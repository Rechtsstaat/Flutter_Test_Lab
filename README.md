...
- 0011 은 **각 플랫폼의 로그인 화면을 그대로** 띄우고, 로그인 뒤 대시보드가 열리면 연동으로 봅니다. 한방은 입력값을 보지 않습니다 — 아이디와 비밀번호는 플랫폼 자신의 폼에서 플랫폼 자신의 엔드포인트로 가고, 세션은 플랫폼만 읽을 수 있는 WebView 쿠키 저장소에 남습니다.
...
| `lib/mirror_session.dart` | 플랫폼 페이지를 담는 WebView 세션, 등록·종료 버튼 감지, 로그인 풀림 감지 |
| `lib/mobile_layout.dart` | 데스크톱 전용 등록 폼을 웹뷰 안에서 세로 한 줄로 되펴는 비파괴 오버레이 |
