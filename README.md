...
통합 매물 입력 폼의 값을 **앱 내부 WebView에 띄운 직방 매물등록 폼 재현 페이지**에 주입하는 Flutter 프로토타입입니다. 실제 공인중개사 계정이나 실제 직방 서버에 접속하지 않습니다.
...
- 번들된 직방 재현 HTML의 입력 요소에 JavaScript로 값이 실제 채워지는지
...
- 직방의 현재 운영 화면/DOM과 재현 HTML의 완전한 일치
...
1. 첫 화면의 통합 매물 입력 폼에는 49개 항목이 있습니다. 필수는 `*`로 표시됩니다.
2. 오른쪽 위 `자동 채우기`를 누르면 필수/선택 항목 모두 테스트 예시 값으로 채워집니다. 이후 각 값은 직접 수정할 수 있습니다.
3. 필수 29개 항목이 모두 채워지면 `직방에 보내기` 버튼이 활성화됩니다.
4. 버튼을 누르면 `assets/zigbang/zigbang_listing_form.html`을 로컬 WebView로 열고 폼 값을 주입합니다.
5. 상단 상태가 `성공: 직방 재현 폼에 N개 값을 입력했습니다.`로 바뀌고 WebView에서 값이 보이면 테스트 성공입니다. `실패:`가 표시되거나 값이 비어 있으면 실패입니다.
6. 결과 화면 왼쪽 위 뒤로가기 버튼으로 통합 폼으로 돌아갈 수 있습니다.

성공 상태는 JavaScript 실행 자체가 아니라 핵심 입력 요소의 값을 DOM에서 다시 읽어 판정합니다. 재현 HTML에 대응하는 실제 입력 컨트롤이 없는 통합 폼 항목은 의도적으로 주입 대상에서 제외됩니다.
...
| `assets/zigbang/zigbang_listing_form.html` | 앱에 번들되는 직방 매물등록 재현 페이지와 값 주입 함수 |
| `pubspec.yaml` | Flutter/WebView 의존성 및 HTML asset 등록 |
| `ios/Runner.xcodeproj/project.pbxproj` | iOS 배포 대상(iOS 15.0) 및 Xcode 서명 설정 |

## 주의

재현 HTML은 테스트용 로컬 asset입니다. 실제 직방의 화면이나 API를 호출하지 않으며, 이 프로젝트로 실제 매물을 등록하지 않습니다. 실제 서비스에 적용하기 전에는 플랫폼의 공식 연동 수단, 이용약관, 개인정보 처리, 사용자 승인 및 에러 복구 방식을 별도로 검토해야 합니다.
